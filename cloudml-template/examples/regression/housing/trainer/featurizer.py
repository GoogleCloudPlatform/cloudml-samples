...
def _extend_feature_columns(feature_columns, args):
    ...
    ZN_bucketized = tf.feature_column.bucketized_column(
        feature_columns["ZN"], boundaries=[0, 10, 100]
    )

    RAD_bucketized = tf.feature_column.bucketized_column(
        feature_columns["RAD"], boundaries=[0, 10, 25]
    )

    TAX_bucketized = tf.feature_column.bucketized_column(
        feature_columns["TAX"], boundaries=[0, 200, 300, 500, 800]
    )

    ZN_bucketized_X_ZN_bucketized = tf.feature_column.crossed_column(
        [ZN_bucketized, RAD_bucketized], 4
    )
    ZN_bucketized_X_TAX_bucketized = tf.feature_column.crossed_column(
        [ZN_bucketized, TAX_bucketized], 8
    )
    RAD_bucketized_X_TAX_bucketized = tf.feature_column.crossed_column(
        [RAD_bucketized, TAX_bucketized], 8
    )

    extended_feature_columns.append(ZN_bucketized)
    extended_feature_columns.append(RAD_bucketized)
    extended_feature_columns.append(TAX_bucketized)
    extended_feature_columns.append(ZN_bucketized_X_ZN_bucketized)
    extended_feature_columns.append(ZN_bucketized_X_TAX_bucketized)
    extended_feature_columns.append(RAD_bucketized_X_TAX_bucketized)
...
def _create_feature_columns():
    ...
    for feature_name in metadata.CATEGORICAL_FEATURE_NAMES_WITH_IDENTITY:
        feature_columns[
            ...
            feature_name,
            num_buckets=metadata.CATEGORICAL_FEATURE_NAMES_WITH_IDENTITY[feature_name],
        )
...
def _get_sparse_and_dense_columns(feature_columns):
    ...
    dense_columns = [
        ...
        if (
            isinstance(column, feature_column.NumericColumn)
            or isinstance(column, feature_column.EmbeddingColumn)
            or isinstance(column, feature_column.IndicatorColumn)
        )
    ]
    ...
    sparse_columns = [
        ...
        for column in feature_columns
        if (
            isinstance(column, feature_column.VocabularyListCategoricalColumn)
            or isinstance(column, feature_column.IdentityCategoricalColumn)
            or isinstance(column, feature_column.BucketizedColumn)
            or isinstance(column, feature_column.CrossedColumn)
        )
    ]
