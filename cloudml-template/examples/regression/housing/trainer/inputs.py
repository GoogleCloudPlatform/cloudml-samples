...
def get_feature_spec(is_serving=False):
    ...
    column_names = (
        metadata.SERVING_COLUMN_NAMES if is_serving else metadata.COLUMN_NAMES
    )
...
def json_serving_input_receiver_fn():
    ...
    for column_name in metadata.SERVING_COLUMN_NAMES:
        if column_name in metadata.CATEGORICAL_FEATURE_NAMES_WITH_IDENTITY:
            receiver_tensors[column_name] = tf.placeholder(shape=[None], dtype=tf.int32)
...
def csv_serving_input_receiver_fn():
    ...
    return tf.estimator.export.ServingInputReceiver(
        features=process_features(features), receiver_tensors={"csv_row": csv_row}
    )
...
def example_evaluating_input_receiver_fn():
    ...
    features = tf.parse_example(tf_example, features=get_feature_spec(is_serving=False))
