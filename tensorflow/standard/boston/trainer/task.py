...
def get_args():
    ...
    parser.add_argument(
        "--train-file", type=str, required=True, help="Dataset file local or GCS"
    )
    parser.add_argument(
        "--test-split",
        type=float,
...
def train_and_evaluate(args):
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: model.input_fn(
            train_data, train_labels, args.batch_size, mode=tf.estimator.ModeKeys.TRAIN
        ),
    ...
    eval_spec = tf.estimator.EvalSpec(
        input_fn=lambda: model.input_fn(
            test_data, test_labels, args.batch_size, mode=tf.estimator.ModeKeys.EVAL
        ),
        steps=None,
        exporters=[exporter],
        start_delay_secs=10,
