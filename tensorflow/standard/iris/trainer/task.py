...
def get_args():
    ...
    parser.add_argument(
        "--learning-rate", help="Learning rate", type=float, default=0.01
    )
    parser.add_argument(
...
def train_and_evaluate(args):
    ...
    train_input_fn = lambda: model.input_fn(
        filename=args.train_files, batch_size=args.train_batch_size, shuffle=True
    )
    train_spec = tf.estimator.TrainSpec(train_input_fn, max_steps=args.train_steps)

    # Define evaluating spec. Don't shuffle evaluation data.
    exporter = tf.estimator.FinalExporter("exporter", model.serving_input_receiver_fn)
    # Create EvalSpec.
    eval_input_fn = lambda: model.input_fn(
        filename=args.eval_files, batch_size=args.eval_batch_size, shuffle=False
    )
    eval_spec = tf.estimator.EvalSpec(
        eval_input_fn, steps=args.eval_steps, exporters=[exporter], name="iris-eval"
    )
