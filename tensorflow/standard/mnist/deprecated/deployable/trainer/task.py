...
def fill_feed_dict(data_set, images_pl, labels_pl):
    ...
    # `batch size` examples.
    _, images_feed, labels_feed = data_set.next_batch(FLAGS.batch_size, FLAGS.fake_data)
    feed_dict = {
...
def run_training():
    ...
    if FLAGS.input_path:
        files = [os.path.join(FLAGS.input_path, file_name) for file_name in INPUT_FILES]
    ...
    with tf.Graph().as_default():
        ...
        build = builder.SavedModelBuilder(os.path.join(FLAGS.model_dir, "saved_model"))
        logging.debug(
            "Saved model path %s", os.path.join(FLAGS.model_dir, "saved_model")
        )
