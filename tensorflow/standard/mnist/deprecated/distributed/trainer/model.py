...
class Model(object):
    ...
    def build_prediction_graph(self):
        ...
        features = {
            "image": tf.FixedLenFeature(shape=[IMAGE_PIXELS], dtype=tf.float32),
            "key": tf.FixedLenFeature(shape=[], dtype=tf.string),
        }
        ...
        tf.add_to_collection("inputs", json.dumps({"examples_bytes": examples.name}))
...
def parse_examples(examples):
    feature_map = {
        "labels": tf.FixedLenFeature(shape=[], dtype=tf.int64, default_value=[-1]),
        "images": tf.FixedLenFeature(shape=[IMAGE_PIXELS], dtype=tf.float32),
    }
