...
def download_files_from_gcs(source, destination):
    ...
    raw_local_files_data_paths = [
        os.path.join(WORKING_DIR, local_file_name)
...
def preprocess(train_data_file, word_index_file, num_words):
    ...
    train_data = pad_sequences(
        train_data, value=word_index["<PAD>"], padding="post", maxlen=SENTENCE_SIZE
    )
    # Standardize the lengths for test.
    test_data = pad_sequences(
        test_data, value=word_index["<PAD>"], padding="post", maxlen=SENTENCE_SIZE
    )
