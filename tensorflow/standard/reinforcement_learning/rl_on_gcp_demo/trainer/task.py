...
def train(config):
    ...
    model_path = os.path.join(config.job_dir, "model/{}.ckpt".format(config.agent))
            ...
            while not done:
                ...
                if done:
                    ...
                    log_metrics(
                        ...
                        summary_ops,
                        summary_vars,
                        metrics=(ep_cnt, ep_reward, ep_steps, actions, noises),
                    )
                    if ep_cnt % config.eval_interval == 0:
                        ...
                        ckpt_path = saver.save(
                            sess, model_path, global_step=global_step
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--critic-lr", help="critic learning rate", type=float, default=2e-4
    )
    parser.add_argument(
        "--actor-lr", help="actor learning rate", type=float, default=1e-4
    )
    parser.add_argument(
        "--gamma", help="reward discounting factor", type=float, default=0.99
    )
    parser.add_argument(
        "--tau", help="target network update ratio", type=float, default=0.001
    )
    parser.add_argument(
        "--sigma", help="exploration noise standard deviation", type=float, default=0.1
    )
    parser.add_argument(
        "--sigma-tilda",
    ...
    parser.add_argument("--c", help="noise cap", type=float, default=0.15)
    parser.add_argument(
        "--grad-norm-clip",
    ...
    parser.add_argument(
        "--buffer-size", help="replay buffer size", type=int, default=1000000
    )
    parser.add_argument("--d", help="target update interval", type=int, default=2)
    parser.add_argument(
        "--warmup-size", help="warm up buffer size", type=int, default=10000
    )
    ...
    parser.add_argument(
        "--eval-interval", help="interval to test", type=int, default=100
    )
    parser.add_argument(
        "--max-to-keep", help="number of model generations to keep", type=int, default=5
    )
    parser.add_argument(
        "--agent", help="type of agent, one of [DDPG|TD3|C2A2]", default="DDPG"
    )
    parser.add_argument(
        "--job-dir", help="dir to save logs and videos", default="./results"
    )
    parser.add_argument(
        "--record-video",
