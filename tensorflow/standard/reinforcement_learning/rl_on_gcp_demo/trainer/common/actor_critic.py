...
class ActorNetwork(object):
    ...
    def __init__(
        ...
        (
            ...
            self.train_op,
        ) = self.create_actor_ops()

    def create_actor_ops(self):
        """Create training related ops."""
        target_update_op = tf.group(
            [
                x.assign(tf.multiply(y, self.tau) + tf.multiply(x, 1.0 - self.tau))
        ...
        clipped_grad = map(
            lambda x: tf.clip_by_norm(tf.div(x, self.batch_size), self.grad_norm_clip),
            actor_gradients,
        )
    ...
    def train(self, obs, a_gradient):
        self.sess.run(
            self.train_op, feed_dict={self.obs: obs, self.action_gradient: a_gradient}
        )
    ...
    def get_target_action(self, obs):
        if np.ndim(obs) == 1:
            obs = np.expand_dims(obs, axis=0)
        return self.sess.run(self.target_action, feed_dict={self.target_obs: obs})
...
class CriticNetwork(object):
    ...
    def __init__(
        self, sess, state_dim, action_dim, learning_rate, tau, gamma, name=None
    ):
        self.sess = sess
        self.s_dim = state_dim
        self.a_dim = action_dim
        ...
        with tf.variable_scope(net_name):
            (self.obs, self.action, self.q_value) = self.create_critic_network()
        self.params = tf.trainable_variables(scope=net_name)
        with tf.variable_scope(net_name + "_target"):
            (
                self.target_obs,
    ...
    def train(self, obs, action, y):
        return self.sess.run(
            self.train_op, feed_dict={self.obs: obs, self.action: action, self.y_ph: y}
        )
    ...
    def get_qval(self, obs, action):
        ...
        return self.sess.run(
            self.q_value, feed_dict={self.obs: obs, self.action: action}
        )
    ...
    def get_target_qval(self, obs, action):
        ...
        return self.sess.run(
            self.target_q_value,
            feed_dict={self.target_obs: obs, self.target_action: action},
        )

    def get_action_gradients(self, obs, actions):
        return self.sess.run(
            self.action_grad, feed_dict={self.obs: obs, self.action: actions}
        )
