...
def create_parser():
    ...
    parser.add_argument(
        "--eval_only_summary_filename",
        type=str,
        help=(
            "If given, model will not be trained but only evaluated using "
    ...
    for index in range(1, 14):
        column = tf.contrib.layers.bucketized_column(
            tf.contrib.layers.real_valued_column(
                FORMAT_INT_FEATURE.format(index), dtype=tf.int64
            ),
            boundaries,
        )
    ...
    if model_type == LINEAR:
        ...
        for index in range(14, 40):
            ...
            embedding = tf.contrib.layers.embedding_column(
                column, embedding_size, combiner="mean"
            )
...
def get_transformed_reader_input_fn(
    transformed_metadata, transformed_data_paths, batch_size, mode
):
    """Wrap the get input features function to provide the runtime arguments."""
    return input_fn_maker.build_training_input_fn(
        metadata=transformed_metadata,
        file_pattern=(
            transformed_data_paths[0]
            if len(transformed_data_paths) == 1
...
def get_vocab_sizes():
    ...
    return {
        FORMAT_CATEGORICAL_FEATURE_ID.format(index): int(10 * 1000)
        for index in range(14, 40)
    }
...
def get_experiment_fn(args):
    ...
    def get_experiment(output_dir):
        ...
        num_partitions = max(
            1,
            1 + cluster.num_tasks("worker")
            if cluster and "worker" in cluster.jobs
            else 0,
        )
        ...
        export_strategy = tf.contrib.learn.utils.make_export_strategy(
            serving_input_fn, exports_to_keep=5, default_output_alternative_key=None
        )
        ...
        return tf.contrib.learn.Experiment(
            estimator=estimator,
            train_steps=(
                args.train_steps or args.num_epochs * train_set_size // args.batch_size
            ),
...
def main(argv=None):
    ...
    if args.eval_only_summary_filename:
        ...
        learn_runner.run(experiment_fn=get_experiment_fn(args), output_dir=output_dir)
