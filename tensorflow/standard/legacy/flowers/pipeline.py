...
SAMPLE_IMAGE = "gs://cloud-ml-data/img/flower_photos/tulips/4520577328_a94c11e806_n.jpg"
...
def process_args():
    ...
    parser.add_argument(
        "--eval_set_size", default=50, help="The size of the eval dataset."
    )
    parser.add_argument(
        "--input_dict",
        default=None,
    ...
    parser.add_argument(
        ...
        default="flowerse2e",
        help=(
            "If --cloud is used, the model is deployed with this "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to Dataflow SDK location. If None, Pip will "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "If specified, preprocessing steps will be skipped."
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "If specified, preprocessing and training steps ares skipped."
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Google Cloud Storage bucket to be used for uploading intermediate " "data"
        ),
    ),
    parser.add_argument(
        "--output_dir",
...
class FlowersE2E(object):
    ...
    def preprocess(self):
        ...
        train_output_prefix = os.path.join(
            self.args.output_dir, "preprocessed", train_dataset_name
        )
        eval_output_prefix = os.path.join(
            self.args.output_dir, "preprocessed", eval_dataset_name
        )

        train_args = (
            train_dataset_name,
            self.args.train_input_path,
            train_output_prefix,
            dataflow_sdk,
            trainer_uri,
        )
        ...
        pipeline_pool = [
            thread_pool.apply_async(self.run_pipeline, train_args),
    ...
    def run_pipeline(
        self, dataset_name, input_csv, output_prefix, dataflow_sdk_location, trainer_uri
    ):
        ...
        options = {
            "staging_location": os.path.join(
                self.args.output_dir, "tmp", dataset_name, "staging"
            ),
            "temp_location": os.path.join(self.args.output_dir, "tmp", dataset_name),
    ...
    def deploy_model(self, model_path):
        ...
        print(
            "Deployed %s version: %s"
            % (self.args.deploy_model_name, self.args.deploy_model_version)
        )
    ...
    def run(self):
        ...
        if not model_path:
            train_prefix, eval_prefix = (
                self.args.preprocessed_train_set,
...
def get_cloud_project():
    cmd = ["gcloud", "-q", "config", "list", "project", "--format=value(core.project)"]
