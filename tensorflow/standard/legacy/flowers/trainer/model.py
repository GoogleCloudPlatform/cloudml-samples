...
class Model(object):
            ...
            with tf.name_scope("inputs"):
                feature_map = {
                    ...
                    "label": tf.FixedLenFeature(
                        shape=[1], dtype=tf.int64, default_value=[self.label_count]
                    ),
    ...
    def restore_from_checkpoint(
        self, session, inception_checkpoint_file, trained_checkpoint_file
    ):
        ...
        inception_vars = {
            var.op.name: var for var in all_vars if var.op.name in var_to_shape_map
        }
    ...
    def export(self, last_checkpoint, output_dir):
        ...
        with tf.Session(graph=tf.Graph()) as sess:
            ...
            self.restore_from_checkpoint(
                sess, self.inception_checkpoint_file, last_checkpoint
            )
