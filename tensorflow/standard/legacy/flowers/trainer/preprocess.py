...
def configure_pipeline(p, opt):
    ...
    _ = (
        ...
        | "Save to disk"
        >> beam.io.WriteToTFRecord(opt.output_path, file_name_suffix=".tfrecord.gz")
    )
...
def default_args(argv):
    ...
    parser.add_argument(
        "--output_path", required=True, help="Output directory to write results to."
    )
    parser.add_argument(
        "--project",
        type=str,
    ...
    if parsed_args.cloud:
        ...
        default_values = {
            ...
            "temp_location": os.path.join(
                os.path.dirname(parsed_args.output_path), "temp"
            ),
