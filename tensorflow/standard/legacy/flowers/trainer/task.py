...
class Evaluator(object):
    ...
    def evaluate(self, num_eval_batches=None):
        ...
        with tf.Graph().as_default() as graph:
            self.tensors = self.model.build_eval_graph(
                self.eval_data_paths, self.eval_batch_size
            )
        ...
        with self.sv.managed_session(
            master="", start_standard_services=False
        ) as session:
            ...
            if self.stream:
                ...
                if not self.batch_of_examples:
                    ...
                    for i in range(num_eval_batches):
                        self.batch_of_examples.append(
                            session.run(self.tensors.examples)
                        )
    ...
    def write_predictions(self):
        ...
        with tf.Graph().as_default() as graph:
            self.tensors = self.model.build_eval_graph(
                self.eval_data_paths, self.batch_size
            )
            self.saver = tf.train.Saver()
        self.sv = tf.train.Supervisor(
            graph=graph,
        ...
        with self.sv.managed_session(
            ...
            with file_io.FileIO(
                os.path.join(self.output_path, "predictions.csv"), "w"
            ) as f:
...
class Trainer(object):
    ...
    def __init__(self, args, model, cluster, task):
        ...
        self.evaluator = Evaluator(
            self.args, self.model, self.args.eval_data_paths, "eval_set"
        )
        self.train_evaluator = Evaluator(
            self.args, self.model, self.args.train_data_paths, "train_set"
        )
    ...
    def run_training(self):
        ...
        with tf.Graph().as_default() as graph:
            with tf.device(device_fn):
                ...
                self.tensors = self.model.build_train_graph(
                    self.args.train_data_paths, self.args.batch_size
                )
    ...
    def eval(self, session):
        ...
        if train_eval_rate < self.min_train_eval_rate and self.last_save > 0:
            logging.info(
                ...
                self.eval_interval,
                self.min_train_eval_rate * eval_time,
            )
    ...
    def save_summaries(self, session):
        self.sv.summary_computed(
            session, session.run(self.summary_op), self.global_step
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inception_checkpoint_file", type=str, default=DEFAULT_INCEPTION_CHECKPOINT
    )
    parser.add_argument(
        "--verbosity",
