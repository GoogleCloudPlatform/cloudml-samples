...
def create_random_movie_samples(
    all_movies, movies_to_exclude, num_movies_to_sample, random_seed
):
    ...
    return (
        ...
        .choice(sorted(candidate_movie_ids), num_movies_to_sample, replace=False)
        .tolist()
    )
...
def make_ratings_schema():
    return _make_schema(
        RATING_COLUMNS, [tf.int64, tf.string, tf.float32, tf.int64], [-1, "", 0.0, -1]
    )


def make_movies_schema():
    return _make_schema(
        MOVIE_COLUMNS, [tf.string, tf.string, [tf.string]], ["", "", None]
    )
...
def make_prediction_schema():
    prediction_columns = [
        column for column in EXAMPLE_COLUMNS if column != LABEL_RATING_SCORE
    ]
    return _make_schema(
        prediction_columns,
        [
...
def make_preprocessing_fn():
    ...
    def preprocessing_fn(inputs):
        ...
        """
        result = {column_name: inputs[column_name] for column_name in EXAMPLE_COLUMNS}
        ...
        def scale_sparse_values(x, min_value, max_value):
            ...
            return tf.SparseTensor(
                indices=x.indices, values=scaled_values, dense_shape=x.dense_shape
            )
        ...
        def map_to_int(x, vocabulary_or_file):
            ...
            if (
                hasattr(impl, "_asset_files_supported")
                and impl._asset_files_supported()
