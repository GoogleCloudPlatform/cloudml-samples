...
def make_query_feature_columns():
    ...
    query_movie_ids = tf.contrib.layers.sparse_column_with_integerized_feature(
        column_name=QUERY_RATED_MOVIE_IDS, bucket_size=MOVIE_VOCAB_SIZE
    )
    ...
    query_genre_ids = tf.contrib.layers.sparse_column_with_integerized_feature(
        column_name=QUERY_RATED_GENRE_IDS, bucket_size=GENRE_VOCAB_SIZE
    )
...
def make_candidate_feature_columns():
    ...
    candidate_movie_id = tf.contrib.layers.sparse_column_with_integerized_feature(
        column_name=CANDIDATE_MOVIE_ID, bucket_size=MOVIE_VOCAB_SIZE
    )

    candidate_genre_ids = tf.contrib.layers.sparse_column_with_integerized_feature(
        column_name=CANDIDATE_GENRE_IDS, bucket_size=GENRE_VOCAB_SIZE
    )
...
def make_feature_columns():
    ...
    feature_columns = make_query_feature_columns() | make_candidate_feature_columns()
    ...
    ranking_candidate_movie_ids = (
        tf.contrib.layers.sparse_column_with_integerized_feature(
            column_name=RANKING_CANDIDATE_MOVIE_IDS, bucket_size=MOVIE_VOCAB_SIZE
        )
    )
...
def make_input_fn(
    ...
    def _input_fn():
        ...
        if not data_file_pattern:
            raise ValueError(
                "data_file_pattern must be set. Value provided: %s" % data_file_pattern
            )
        ...
        file_pattern = (
            data_file_pattern[0] if len(data_file_pattern) == 1 else data_file_pattern
        )
...
def generate_top_k_scores_and_ids(logits, top_k):
    ...
    top_k_scores, top_k_ids = tf.nn.top_k(input=probabilities, k=top_k)
    ...
    predictions = {
        ...
        tf.contrib.learn.PredictionKey.PROBABILITIES: top_k_scores,
        # served as "classes" by Servo in the ClassificationResult
        tf.contrib.learn.PredictionKey.CLASSES: top_k_ids,
    }
    output_alternatives = {
...
def model_builder(hparams):
    ...
    def _matrix_factorization_model_fn(features, target_ratings, mode):
        ...
        candidate_biases, _, _ = tf.contrib.layers.weighted_sum_from_feature_columns(
            columns_to_tensors=features,
        ...
        if mode != tf.contrib.learn.ModeKeys.INFER:
            ...
            predictions = tf.reduce_sum(
                tf.multiply(query_embeddings, candidate_embeddings), 1, keep_dims=True
            )
                ...
                if hparams.eval_type == REGRESSION:
                    ...
                    if hparams.use_ranking_candidate_movie_ids:
                        ...
                        predictions = tf.where(
                            movies_to_rank_condition,
                            predictions,
                            tf.fill(tf.shape(predictions), tf.reduce_min(predictions)),
                        )
                    return tf.contrib.learn.ModelFnOps(
                        mode=mode, predictions=predictions, loss=loss
                    )
            ...
            scores = tf.matmul(
                query_embeddings, candidate_movie_embedding_weights, transpose_b=True
            )
            if hparams.enable_bias:
                biases = tf.add(query_biases, candidate_biases)
            ...
            pruned_scores = tf.where(
                ...
                tf.fill(tf.shape(scores), tf.reduce_min(scores)),
                scores,
            )
    ...
    def _embed_query_features(features, mode):
        ...
        bottom_layer = tf.concat(
            [movies_embedding, genres_embedding_freqs, genres_embedding_ratings],
            1,
        ...
        if hparams.enable_batch_norm:
            ...
            normalizer_params = {"is_training": mode == tf.contrib.learn.ModeKeys.TRAIN}
        else:
            normalizer_fn = None
    ...
    def _dnn_softmax_fn(features, targets, mode):
        ...
        tf.summary.scalar(
            "query_embeddings_zero_fraction", tf.nn.zero_fraction(query_embeddings)
        )

        # Create layers for target features.
        if mode != tf.contrib.learn.ModeKeys.INFER:
            logits_layer = (
                tf.matmul(query_embeddings, tf.transpose(class_weights)) + class_biases
            )
            ...
            if mode == tf.contrib.learn.ModeKeys.TRAIN:
                ...
                if hparams.eval_type == REGRESSION:
                    ...
                    predictions = (
                        tf.matmul(query_embeddings, tf.transpose(class_weights))
                        + class_biases
                    )
                    if hparams.use_ranking_candidate_movie_ids:
                        # Get ranking candidate movie ids to rank our candidate movie
...
def create_evaluation_metrics(eval_type):
    ...
    if eval_type == REGRESSION:
        ...
        for k in [1, 5, 10, 20, 35, 50, 75, 100]:
            eval_metrics["precision_at_%02d" % k] = tf.contrib.learn.MetricSpec(
                metric_fn=functools.partial(
                    tf.contrib.metrics.streaming_sparse_precision_at_k, k=k
                )
            )
            eval_metrics["recall_at_%02d" % k] = tf.contrib.learn.MetricSpec(
                metric_fn=functools.partial(
        ...
        for k in [1, 2, 3, 4, 5, 10, 20]:
            eval_metrics[
                "mean_average_precision_at_%d" % k
            ] = tf.contrib.learn.MetricSpec(
                metric_fn=functools.partial(
...
def make_experiment_fn(args):
    ...
    def make_experiment(output_dir):
        ...
        export_strategy = tf.contrib.learn.utils.make_export_strategy(
            serving_input_fn, default_output_alternative_key=DEFAULT_OUTPUT_ALTERNATIVE
        )
        ...
        return tf.contrib.learn.Experiment(
            ...
            train_steps=(
                args.train_steps
                or args.num_epochs * args.train_set_size // args.batch_size
            ),
...
def create_parser():
    ...
    parser.add_argument(
        "--eval_type", help="Evaluation type", choices=EVAL_TYPES, default=REGRESSION
    )
    parser.add_argument("--train_data_paths", type=str, action="append", required=True)
    parser.add_argument("--eval_data_paths", type=str, action="append", required=True)
    ...
    parser.add_argument(
        "--train_set_size", help="Number of samples on the train dataset.", type=int
    )
    ...
    parser.add_argument(
        "--optimizer", help="Optimizer to use", choices=OPTIMIZERS, default=ADAGRAD
    )
    parser.add_argument(
        "--enable_batch_norm",
        action="store_true",
...
def main(argv=None):
    ...
    learn_runner.run(experiment_fn=make_experiment_fn(args), output_dir=output_path)
