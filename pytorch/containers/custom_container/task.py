...
def train(net, train_loader, optimizer, epoch):
    ...
    for batch_index, data in enumerate(train_loader):
        ...
        if batch_index % 6 == 5:  # print every 6 mini-batches
            print("[%d, %5d] loss: %.3f" % (epoch, batch_index + 1, running_loss / 6))
            running_loss = 0.0
...
def test(net, test_loader):
    ...
    print(
        "\tAccuracy: {}/{} ({:.0f}%)\n".format(
            ...
            100.0 * correct / (len(test_loader) * test_loader.batch_size),
        )
    )
...
def train_model(args):
    ...
    # Open our dataset
    train_loader, test_loader = data_utils.load_data(args.test_split, args.batch_size)
