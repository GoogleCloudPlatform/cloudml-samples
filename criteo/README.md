...
This sample has been deprecated. It has not been tested against the newest version of TensorFlow. When running `gcloud ml-engine` commands be sure to use `--runtime-version 0.12`. Additionally the Google Cloud Machine Learning SDK, has been superseded by [Tensorflow Transform](https://github.com/tensorflow/transform). For an example that uses this new library check out [The Criteo TF Transform example](../criteo_tft)
