...
def vocabulary_file_by_name(working_dir, key):
    ...
    """
    return os.path.join(working_dir, transform_fn_io.TRANSFORM_FN_DIR, "assets", key)
        ...
        if key in STRING_TO_INT_FEATURE_KEYS:
            if not key in CATEGORICAL_FEATURE_KEYS_TO_BE_REMOVED:
                categorical_valued_columns[
                    ...
                    key=key, num_buckets=vocabulary_size_by_name(tft_working_dir, key)
                )
            ...
            key.endswith("_bucketized")
            and key.replace("_bucketized", "") in TO_BE_BUCKETIZED_FEATURE
        ):
            categorical_valued_columns[
                key
            ] = tf.feature_column.categorical_column_with_identity(
    ...
    real_valued_columns.update(
        {
            ...
            "native_country_embedding": tf.feature_column.embedding_column(
                categorical_valued_columns["native_country"], dimension=embedding_size
            ),
