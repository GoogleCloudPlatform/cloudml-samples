...
def _make_training_input_fn(tft_working_dir,
  ...
  # Examples have already by transformed so we only need the feature_columns
  ...
  def input_fn():
    ...
    #See https://github.com/tensorflow/tensorflow/blob/master/tensorflow/docs_src/performance/datasets_performance.md
    files = tf.data.Dataset.list_files(os.path.join(tft_working_dir, filebase + '*'))
    dataset = files.interleave(lambda x : tf.data.TFRecordDataset(x).map(parser,
