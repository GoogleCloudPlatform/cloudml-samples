...
if __name__ == '__main__':
  ...
  if args.cloud:
    ...
    beam_args = {
        'job_name': 'cloudml-sample-molecules-preprocess-' + timestamp(),
  ...
  print('Credentials: {}'.format(os.environ['GOOGLE_APPLICATION_CREDENTIALS']))
  print()
  ...
  # 3) Preprocessing
  print('Preprocessing')
  train_input_fn, eval_input_fn, serving_input_fn = preprocess.run(
      INPUT_SCHEMA,
      LABELS,
      PubChemToInputData(raw_data_files_pattern),
      normalize_inputs,
  ...
  if not args.cloud:
    ...
    # Running in Cloud ML Engine. Pickle everything to pass in to the trainer
    pickle_dir = os.path.join(work_dir, 'pickle')
    if not tf.gfile.Exists(pickle_dir):
      tf.gfile.MakeDirs(pickle_dir)

    train_input_fn_file = os.path.join(pickle_dir, 'train_input_fn')
    trainer.dump(train_input_fn, train_input_fn_file)

    eval_input_fn_file = os.path.join(pickle_dir, 'eval_input_fn')
    trainer.dump(eval_input_fn, eval_input_fn_file)

    serving_input_fn_file = os.path.join(pickle_dir, 'serving_input_fn')
    trainer.dump(serving_input_fn, serving_input_fn_file)

    estimator_file = os.path.join(pickle_dir, 'estimator')
    trainer.dump(estimator, estimator_file)

    # Spawn a gcloud subprocess to submit the training job to ml-engine
    job = 'cloudml_sample_molecules_training_' + timestamp()

    assert work_dir.startswith('gs://'), '--work-dir must be in GCS'
    bucket = 'gs://' + work_dir.split('/')[2]
    ...
    cmd = [
        ...
        '--region', args.region,
        '--',
        '--train-input-fn', train_input_fn_file,
        '--eval-input-fn', eval_input_fn_file,
        '--serving-input-fn', serving_input_fn_file,
        '--estimator', estimator_file,
