...
def model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        label_prediction_loss = tf.losses.softmax_cross_entropy(
            onehot_labels=onehot_labels, logits=label_predictor_logits(source_features)
        )

        # There are two domains, 0: source and 1: target
        domain_labels = tf.concat(
            (
                tf.zeros(source.shape[0], dtype=tf.int32),
                tf.ones(target.shape[0], dtype=tf.int32),
            ),
            axis=0,
        )
    ...
    if params["use_tpu"]:
        ...
        def host_call_fn(gs, lpl, dcl, ls):
            ...
            with summary.create_file_writer(
                params["model_dir"], max_queue=params["save_checkpoints_steps"]
            ).as_default():
...
def train_input_fn(params={}):
    ...
    source_labels_dataset = (
        tf.data.Dataset.from_tensor_slices((source_tensor, labels_tensor))
        .repeat()
        .shuffle(32)
    )
    target_dataset = (
        tf.data.Dataset.from_tensor_slices(target_tensor).repeat().shuffle(32)
    )
