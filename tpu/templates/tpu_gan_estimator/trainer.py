...
def gen_model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        gan_train_ops = tf.contrib.gan.gan_train_ops(
            gan_model, gan_loss, optimizer, dummy_optimizer
        )
...
def dis_model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        gan_train_ops = tf.contrib.gan.gan_train_ops(
            gan_model, gan_loss, dummy_optimizer, optimizer
        )

        # tf.contrib.gan's train op does not manage global steps in it
        train_op = tf.group(
            gan_train_ops.discriminator_train_op, global_step.assign_add(1)
        )
...
def main(args):
    ...
    if args.use_tpu:
        ...
        gen_config = tf.estimator.RunConfig(
            model_dir=os.path.join(args.model_dir, "generator")
        )
        dis_config = tf.estimator.RunConfig(
            model_dir=os.path.join(args.model_dir, "discriminator")
        )

        gen_estimator = tf.estimator.Estimator(
            gen_model_fn, config=gen_config, params=params
        )

        dis_estimator = tf.estimator.Estimator(
            dis_model_fn, config=dis_config, params=params
        )
