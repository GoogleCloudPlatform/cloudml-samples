...
def colab_url(filename):
    return "https://colab.research.google.com/github/GoogleCloudPlatform/cloudml-samples/blob/master/tpu/templates/{}".format(
        filename
    )
...
def format(path):
    ...
    for ipynb_filename in ipynb_filenames:
        formatted += "[{}]({}) [[Colab]]({})<br>".format(
            ipynb_filename.split("/")[-1], ipynb_filename, colab_url(ipynb_filename)
        )
...
for row in rows:
    table_str += "\n {} | ".format(row)
    table_str += " | ".join(
        [format(samples[row].get(column, "")) for column in columns]
    )
