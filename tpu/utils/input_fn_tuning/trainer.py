...
def dataset_parser(value):
    ...
    label = (
        tf.cast(tf.reshape(parsed["image/class/label"], shape=[]), dtype=tf.int32) - 1
    )
...
def _train_input_fn(params, input_fn_params):
    ...
    filenames_dataset = filenames_dataset.apply(
        tf.contrib.data.shuffle_and_repeat(buffer_size=32)
    )

    def fetch_tfrecords(filenames):
        dataset = tf.data.TFRecordDataset(
            filenames,
            buffer_size=1e6 * input_fn_params["tfrecord_dataset_buffer_size"],
...
def main(args):
    ...
    input_fn_params = {
        input_fn_param_name: getattr(args, input_fn_param_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_dir", type=str, default="/tmp/tpu-template")
