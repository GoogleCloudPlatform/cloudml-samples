...
def delete_tpu_and_wait(env, sleep_time=10):
    ...
    while (
        nodes
        and "nodes" in nodes
        and tpu_name in [n["name"].split("/")[-1] for n in nodes["nodes"]]
    ):
...
def build_submit_script(input_fn_params):
    ...
    input_fn_params_str = " ".join(
        ["--{}={}".format(k, v) for k, v in input_fn_params.items()]
    )
...
def make_profile_tpu(subprocess_env):
    def profile_tpu(**input_fn_params):
        ...
        p_submit = Popen(
            submit_command, stdout=PIPE, preexec_fn=os.setsid, env=subprocess_env
        )
        ...
        while returncode is None and len(scores) < n_scores and counter < n_attempts:
            ...
            p_trace = Popen(
                trace_command, stdout=PIPE, preexec_fn=os.setsid, env=subprocess_env
            )
            counter += 1
            ...
            trace_filenames = tf.gfile.Glob(
                "{}/plugins/profile/**/input_pipeline.json".format(model_dir)
            )
            ...
            if trace_filenames:
                ...
                for trace_filename in trace_filenames:
                    ...
                    try:
                        infeed_percent_average = float(
                            input_pipeline[0]["p"]["infeed_percent_average"]
                        )
        ...
        print(
            ">>>>> training process finished with returncode: {}, number of attempts: {}, number of scores: {}".format(
                returncode, counter, len(scores)
            )
        )
        ...
        params_scores = {
            "input_fn_params": {
                k: int("{}".format(v)) for k, v in input_fn_params.items()
            },
            "scores": scores,
            "score": score,
        }
...
def main(args):
    ...
    gp_minimize(
        profile_tpu, space, n_calls=args.n_calls, n_random_starts=5, x0=x0, y0=y0
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--location", type=str, default="europe-west4-a")
    parser.add_argument("--tpu-name", type=str, default="input_fn_tuning_tpu")
    parser.add_argument("--output-dir", type=str, default="gs://your-gcs-bucket/path")
